@extends('dashboard-user.layouts.main') @section('container')

<div class="container">
    <div class="row my-3">
        <div class="col-lg-8">
            <h1 class="mb-3">{{ $post->title }}</h1>
            <a href="/dashboard-user/posts" class="btn btn-success">
                <span data-feather="arrow-left" class="align-text-bottom"></span>
                Back to All My Post</a>
            <a href="/dashboard-user/posts/{{ $post->slug }}/edit" class="btn btn-warning">
                <span data-feather="edit" class="align-text-bottom"></span>
                Edit</a>
            <form
                action="/dashboard-user/posts/{{ $post->slug }}"
                method="post"
                class="d-inline">
                @method('delete') @csrf
                <button
